...
namespace Lockstep.Framework.FastCollections
{
	...
	public class FastBucket<T>
	{
		...
		private readonly FastStack<int> _openSlots = new FastStack<int>();
		...
		{
			int index = _openSlots.Count > 0 ? _openSlots.Pop() : PeakCount++;
		...
		public void InsertAt(T item, int index)
		{
			...
			if (index < arrayAllocation.Length && arrayAllocation.Get(index))
			{
				...
				if (index < PeakCount)
				{
					int indexIndex = Array.BinarySearch<int>(_openSlots.innerArray, index);
					Shortcuts.Shift(_openSlots.innerArray,indexIndex, _openSlots.innerArray.Length, -1);
				}
				else if (index >= PeakCount)
				{
					for (; PeakCount < index; PeakCount++)
					{
						_openSlots.Add(PeakCount);
					}
					PeakCount++;
				}
