...
namespace Lockstep.Framework.Pathfinding
{
	public class GridNode
	{
		...
		{   
			...
			WorldPos.X = gridX * Fix64.One + GridManager.OffsetX;
			WorldPos.Y = gridY * Fix64.One + GridManager.OffsetY;
		...
		private void GenerateNeighbors ()
		{
			...
			for (i = -1; i <= 1; i++) {
				...
				for (_j = -1; _j <= 1; _j++) {
					if (i == 0 && _j == 0) //Don't do anything for the same node
                        continue;
					_checkY = gridY + _j;
					if (GridManager.ValidateCoordinates (_checkX, _checkY)) {
						int neighborIndex;
						if ((i != 0 && _j != 0)) {
							//Diagonal
						...
						GridNode checkNode = GridManager.Grid [GridManager.GetGridIndex (_checkX, _checkY)];
		...
		static int i, _j, _checkX, _checkY;
