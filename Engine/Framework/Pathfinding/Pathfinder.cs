...
namespace Lockstep.Framework.Pathfinding
{
    public static class Pathfinder
    {
        ...
        public static void SmoothPath(FastList<GridNode> nodePath, Vector2 End, FastList<Vector2> outputVectorPath, int unitHalfSize)
        {
        ...
        public static bool FindPath(Vector2 End, GridNode startNode, GridNode endNode, FastList<Vector2> outputVectorPath, int unitHalfSize = 1, uint combinePathsVersion = 0)
        {
			outputPathBuffer.FastClear ();
        ...
        public static bool GetPathNodes(Fix64 StartX, Fix64 StartY, Fix64 EndX, Fix64 EndY, out GridNode startNode, out GridNode endNode)
        {
        ...
        private class AlternativeNodeFinder
        {
            ...
            public void SetValues(Vector2 worldPos, int xGrid, int yGrid, int maxTestDistance)
            {
                XGrid = xGrid;
                YGrid = yGrid;
            ...
            public GridNode GetNode ()
            {
                ...
                Fix64 xDif = (OffsettedPos.X - XGrid);
                xDif = xDif.ClampOne();
                Fix64 yDif = (OffsettedPos.Y - YGrid);
                yDif = yDif.ClampOne();
                Fix64 nodeHalfWidth = (Fix64.One / 2);
            ...
            Fix64 closestDistance;
            void CheckPathNode(GridNode node)
            {
                if (node != null && node.Unwalkable == false)
                {                
                    Fix64 distance = node.WorldPos.FastDistance(this.WorldPos);
        ...
        /// <returns></returns>
		public static bool GetEndNode(Vector2 from, Vector2 dest, out GridNode outputNode)
		{
            ...
            if (outputNode == null)
            {
                ...
                foreach (var coordinate in FractionalLineAlgorithm.Trace(
                    dest.X.ToDouble(), dest.Y.ToDouble(), from.X.ToDouble(), from.Y.ToDouble()))
                {
                    outputNode = GridManager.GetNode(
                        new Fix64(coordinate.X), new Fix64(coordinate.Y));
                    if (outputNode != null)
                    {
                        return true;
                    }
        ...
        /// <param name="returnNode"></param>
        /// <returns></returns>
        public static bool GetStartNode(Vector2 dest, out GridNode returnNode)
        {
        ...
        public static bool StarCast(Vector2 dest, out GridNode returnNode)
        {
            int xGrid, yGrid;
        ...
        public static bool GetClosestViableNode(Vector2 from, Vector2 dest, int pathingSize, out GridNode returnNode)
        {
            returnNode = GridManager.GetNode(dest.X, dest.Y);
            if (returnNode.Unwalkable)
            {
                bool valid = false;
                FractionalLineAlgorithm.Coordinate cacheCoord = new FractionalLineAlgorithm.Coordinate();
                bool validTriggered = false;
                pathingSize = (pathingSize + 1) / 2;
                int minSqrMag = pathingSize * pathingSize;
                minSqrMag *= 2;
                foreach (var coordinate in FractionalLineAlgorithm.Trace(dest.X.ToDouble(), dest.Y.ToDouble(), from.X.ToDouble(), from.Y.ToDouble()))
                {
                    currentNode = GridManager.GetNode(new Fix64(coordinate.X), new Fix64(coordinate.Y));
                    if (!validTriggered)
                    {
